...
(ref:robustness-methods) **Robustness to estimation method**: The figure shows the comparation between the non-asset-market uncertainty index using quasi-maximum likelihood, principal component and two-step. All the indexes have been standardized to make proper comparisons.
...
In the same way I employ different variable sets and orderings of the VAR estimated in [Estimates on the Impact of Non-Asset-Market Uncertainty Shocks]. First, I change the sector of the variables to the manufacturing sector with eight variables following @bloom:2009. The set the variables in the estimation are industrial production in manufacturing, employment in manufacturing, average hours in manufacturing, consumer price index, average hourly earnings for production workers in manufacturing, federal funds rate, stock-market index and non-asset-market uncertainty.
...
Henceforth, Figures \@ref(fig:production-medium) and \@ref(fig:employment-medium) confirm the robustness of these VAR results to a range of alternative approaches over variable ordering and variable inclusion.
...
Evaluation of the forecasting capabilities of the different models is carried on with multistep-ahead forecast, computed by iterating forward the reduced form VAR, in the Table \@ref(tab:rmsfe-non-asset-market). Because the ultimate test of a forecasting model is its out-of-sample performance, \@ref(tab:rmsfe-non-asset-market) focuses on pseudo out-of-sample forecasts over the period from 1985:M1 to 2017:M7 ---that is the \% 10 of the sample. It examines forecast horizonts of one to twenty-four quarters. The forecast $h$ steps ahead is computed by estimating the VAR through a given month, making the forecast $h$ steps ahead, reestimating the VAR through the next month, making the next forecast and so on through the forecast period.
...
kable(
  ...
  pack_rows(index = c(
    "Non-asset-market uncertainty index" = 8, 
    "Economic policy uncertainty index" = 8, 
    "Macroeconomic uncertainty index" = 8)
  , latex_gap_space = "1em"
  ) %>% 
  landscape() 
```
