...
(ref:scree-plot) **Scree Plot for Uncertainty Dataset, 1985--2017**: The Figure shows a scree plot with the variance explained by each factor. The percentage explained taper off after the first factor ---which is fill in black.
...
tibble(
  ...
  Parallel = c("choosen", rep("others", length(uncertainty_indexes[-1]) - 1))
) %>% 
...
The persistence of the non-asset-market uncertainty index in the second sample is greater, for example, that the macroeconomic uncertainty from @juraludvng:2015: `r comma(DescribeIndex(uncertainty_indexes[["MU_JLN_h1"]])[4])` versus `r comma(DescribeIndex(DFM_base[DFM_base[["Date"]] >= "2007-08-31", ][["NAM"]])[4])`. This shows that the level of uncertainty is continual in the sample.
...
I explore the dynamic relationship between me uncertainty index and some variables of the United States' economy. For that purpose, I use a benchmark model well documented: the model proposed by @chrieichevan:2005---and used as a base in @chulguilurib:2017. First, the characterization of monetary policy is defined by:
...
Considering $Y_{t}$ as the set of variables to which the dynamic effect of a uncertainty shock will be analyzed, there is:
...
The vector $Y_{1t}$ is composed of the variables whose values at the moment $t$ are contained in $\Omega_{t}$ and it is assumed that they do not respond simultaneously to a monetary policy shock: production, employment, consumption, inflation, new orders, wages and labor. The vector $Y_{2t}$ consists of the values at the moment $t$ of all the other variables in $\Omega_{t}$, which simultaneously respond to a monetary policy shock: stock market index and growth rate of M2. In the last position, I place me uncertainty index $U_{t}$---as do @juraludvng:2015, @bloom:2009, and @chulguilurib:2017. I estimate a VAR with 12 fixed lags, as opposed to the four quarters used in @chrieichevan:2005 following the caveats of Leeb \& Pötscher [-@leebpots:2005;-@leebpots:2006], as discussed in [Lag order]. I recover the structural innovations by means of a Cholesky factorization of the variance-covariance matrix. Since depending on whether the variables react or not to other variables contemporaneously the Cholesky descomposition implies a certain ordering, the estimation order from more exogenous to more endogenous is the same as the presentation of the variables.
...
Production reacts negatively to a uncertainty shock and the persistence last until the second year. Likewise, employment decreases following a uncertainty shock and the impact persists for the same time that production (two years). In contrast, new orders reacts rapidly and last for only one year after which becomes zero. Consumption reacts slowly to the uncertainty shock, this decreases after one year and maintain its behaviour for two years and a half (that is, six months more than production, employment and new orders). The stock market index is severely affected and his persistence last for the same period that the variables discussed so far. Although, the series does not stabilize at a lower level as reported in @chulguilurib:2017. In adittion, the federal funds rate also seems sensitive to an uncertainty shock. Specifically, the Federal reserve reduce the interest rate---which is significant for the first year---that shows that the reduction in the stock market index is due to uncertainty and not movements in the interest rate. Finally, I don't find any evidence of the *rebound* effect evidenced in @bloom:2009 for production neither the *rebound* effect proposed in @chulguilurib:2017 for New Orders.

The Figure \@ref(fig:fevd-plot) shows the forecast error variance descomposition (FEVD) which allows to analyze the contribution of each variable to the $h$-step forecast error variance of all other variables. Accordingly, the forecast error of the production serie is explained by uncertainty six months after the innovation in 10 \%, twelve months after in 25 \%  and twenty-four months after in 30 \%. For employment, uncertainty accounts for 10 \% twelve months after the original structural innovation and 20 \% two years later. For new orders, the uncertainty accounts for 15 \% of the variance six months on and then stabilice in 10 \%. For consumption the uncertainty accounts for less that 5 \% in the first twelve months but after its accounts for 15 \% of the variance. For the financial prices---stock market index---, the uncertainty accounts for the same amount as consumption, i.e., starts in 5 \% and then its stabilizes in 15 \%. Finally, the forecast error of federal funds rate is explained by uncertainty between the 5 \% and 10 \%. These results confirm the importance of uncertainty as a driver of the economy's dynamics.


```{r fevd, warning = FALSE}
...
fevd_tibble %<>% 
  ...
  rename(
    ...
    `Federal Funds Rate` = FederalFundsRate_lin_cycle,
    `Stock Market Index` = StockMarketIndex_log_cycle,
    `Growth Rate of M2` = M2_cca_cycle,
...
(ref:fevd-plot) **Forecast Error Variance Decomposition in the eleven variables VAR**: The figure shows the percentage of variance of the $h$-step-ahead forecast error due to a shocks of each variable, for $h = 1, \ldots, 60$.

```{r fevd-plot, fig.asp = 1.7, fig.cap = "(ref:fevd-plot)", fig.scap = "Forecast Error Variance Decomposition in the eleven variables VAR"}
fevd_tibble %>%
...
(ref:R-plot) **Dynamics of Policy Intervention and Uncertainty in the eleven variable VAR**: The Figure shows the reaction of uncertainty to an unexpected loosening of monetary policy (model with 12 fixed lags following Leeb \& Pötscher [-@leebpots:2005;-@leebpots:2006], as explained in [Lag order]), based on 1000 replications. The axis is in units. The estimation period runs from January 1985 to July 2017. Confidence bands (86 \%) are calculated using bootstrapping techniques as explained in @efrotibs:1993.

```{r R-plot, fig.cap = "(ref:R-plot)", fig.scap = "Dynamics of Policy Intervention and Uncertainty in the eleven variable VAR"}
irf_R_matrix %>%
  ggplot(aes(Months, Response)) +
    geom_hline(aes(yintercept = 0), size = 0.125) +
    geom_ribbon(
...
(ref:R-fevd-plot) **Forecast Error Variance Descomposition for Uncertainty in the Eleven Variable VAR**: The figure shows the fractions of the forecast error variance due to loosening of monetary policy. The estimation period runs from January 1985 to July 2017. 

```{r R-fevd-plot, fig.cap = "(ref:R-fevd-plot)", fig.scap = "Forecast Error Variance Descomposition for Uncertainty in the Eleven Variable VAR"}
